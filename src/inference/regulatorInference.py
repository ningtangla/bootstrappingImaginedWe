...
class GetActionLikelihoodOnIndividualPlanning:
    ...
    def __call__(self, actionDist, action):
        indivualAction = action[self.indivualIndex]
...
class GetPartnerActionLikelihoodOnJointPlanning:
    def __init__(self, numAgentInWe, selfIndex):
        self.weIndex = list(range(numAgentInWe))
        self.selfIndex = selfIndex
    
    def __call__(self, actionDist, action):
        selfAction = action[self.selfIndex]
        weAction = [action[Index] for Index in self.weIndex]
